...
public function course_edited(Request $request) {
	...
	// Returns first admission form view.
			...
			if(!in_array($ext,$allowed) ) {
	   		...
	   		while (($fileop = fgetcsv($files, 1000, ",")) !== FALSE) 
	   		{
	   			if($fileop[0] != "" && $fileop[1] == "")
	   			{
	   				...
	   				$if_programme = strtolower(preg_replace('!\s+!', ' ', $field[0]));
	   				$programme = $this->model->verifyprogramme($if_programme);
	...
	public function admitStudent(Request $request)
	{
		...
		// Store last forms's data in a session
	    session([
	   	...
	   	//Pull session values from all forms
	   	...
	   	if($this->model->enrollStudent($yearOfStudy,$admissionNumber,$campus,$regNumber,$leveOfStudy,$mannerOfEntry,$sponsor,$faculty,$graddate,$program,$lastname,$middlename,$firstname,$sex,$dateOfBirth,$homeDistrict,$ta,$homeVillage,$nationality,$studentStatus,$religion,$maritalStatus,$disability,$paddress,$phone,$email,$bankName,$bankAccount,$parentName,$relationship,$occupation,$parentAddress,$parentEmail,$parentPhone,$schoolName,$examNumber,$yearCompleted, $currentaddress, $bankbranchname))
	    {
	   	...
	   	{
	   		return redirect('/Policy/EnrollStudent')->with('feedback','Student could not be enrolled. '.$regNumber.' already exist.');
	   	}
	}
      ...
      //   return 'Is not set';
   		...
   		public function getstudentlist(Request $request)
   		{
	    	...
	    	if($request['search'] && ($request['search'] != ''))
	    	{
	    		...
	    		$students = $this->model->searchitem($table, $keyword);
	    	}else
	    	{
	    ...
	    */
	    ...
	    {
	    	$table = 'studentDetails';
	    	$results = $this->model->searchitem($table, $request->key);
	    	if(count($results) > 0)
	    	{
	    		foreach ($results as $result) {
	    			echo "<a  data-toggle='modal' data-target='#view-".$result->Id."' style='cursor:pointer'>".$result->Name."</a><br>";
	    		}
	    	}else{
	    ...
	    {
	    	$id = $request->userID;
	    	$count = $this->model->verifyuser($id);
	    	foreach ($count as $value) {
	    		$number = $value->count;
	    	}
	    	echo $number;
	    ...
	    public function actionOnstudents(Request $request)
	    {
	    	...
	    	switch($choice){
	    		case 2:
	    			$fieldsToExport = $request['export'];
	    			$output = "";
	     			foreach ($studentIDs as $studentID) 
	    			{
	    				$students = $this->model->getStudentDetails($studentID);
		    			for($i = 0; $i < count($fieldsToExport);$i++)
		    			{
		    				$output .= $students->$fieldsToExport[$i]."\t";
		    			}
	    				$output .= "\n";
	    			}
	    		...
	    		case 1:
	    			...
	    			foreach ($studentIDs as $studentID) 
	    			{
	    				if($this->model->deletestudent($studentID))
	    				{
	    					array_push($deleted, $studentID);
	    				}
			    	}
			    	if($count == count($deleted))
			    	{
			    		return redirect()->back()->with('feedback', $count.' Record(s) ha(s/ve) been deleted');
			    	}else{
			    		return redirect()->back()->with('feedback', 'Action could not be performed');
			    	}
	     ...
	     public function actionOnsponsor(Request $request)
	    {
	    	$checkbox=$request['checkbox'];
	    	$choice = $request['choice'];
	    	...
	    	switch($choice){
	    		case 2:
	    			...
	    			$output = "";
	     			foreach ($checkbox as $check) 
	    			{
	    				$Sponsors = $this->model->getSponsorDetails($check);
		    			for($i = 0; $i < count($fieldsToExport);$i++)
		    			{
		    				$output .= $Sponsors->$fieldsToExport[$i]."\t";
		    			}
	    				$output .= "\n";
	    			}
	    		...
	    		case 1:
	    			foreach ($checkbox as $check) {
			    		$this->model-> delete_sponsor($check);
			    	}
	      ...
	      public function actionOnfaculty(Request $request)
	    {
	    	$checkbox=$request['checkbox'];
	    	$choice = $request['choice'];
	    	...
	    	switch($choice){
	    		case 2:
	     			...
	     			foreach ($checkbox as $check) 
	    			{
	    				...
	    				$faculties = $this->model->getFacultyDetails($check);


		    			for($i = 0; $i < count($fieldsToExport);$i++)
		    			{
		    				$output .= $faculties->$fieldsToExport[$i]."\t";
		    			}
	    				$output .= "\n"; 
	    			}
	    		...
	    		case 1:
	    			foreach ($checkbox as $check) {
			    		$this->model-> delete_faculty($check);
			    	}
			    	return redirect()->back();
	    			break;
	    		default:
	     ...
	     public function actionOnprogramme(Request $request)
	    {
	    	$checkbox=$request['checkbox'];
	    	$choice = $request['choice'];
	    	...
	    	switch($choice){
	    		case 2:
	     			...
	     			foreach ($checkbox as $check) 
	    			{
	    				...
	    				$prog = $this->model->getProgrammeDetails($check);


		    			for($i = 0; $i < count($fieldsToExport);$i++)
		    			{
		    				$output .= $prog->$fieldsToExport[$i]."\t";
		    			}
	    				$output .= "\n"; 
	    			}
	    		...
	    		case 1:
	    			foreach ($checkbox as $check) {
			    		$this->model-> delete_programme($check);
			    	}
			    	return redirect()->back();
	    			break;
	    		default:
	     ...
	     public function actionOninstitution(Request $request)
	    {
	    	$checkbox=$request['checkbox'];
	    	$choice = $request['choice'];
	    	...
	    	switch($choice){
	    		case 2:
	    			...
	    			$output = "";
	     			foreach ($checkbox as $check) 
	    			{
	    				$institutions = $this->model->getInstitutionDetails($check);
		    			for($i = 0; $i < count($fieldsToExport);$i++)
		    			{
		    				$output .= $institutions->$fieldsToExport[$i]."\t";
		    			}
	    				$output .= "\n";
	    			}
	      ...
	      public function actionOnscholarship(Request $request)
	    {
	    	$checkbox=$request['checkbox'];
	    	$choice = $request['choice'];
	    	...
	    	switch($choice){
	    		case 2:
	    			...
	    			$output = "";
	     			foreach ($checkbox as $check) 
	    			{
	    				$Scholarships = $this->model->getScholarshipDetails($check);
		    			for($i = 0; $i < count($fieldsToExport);$i++)
		    			{
		    				$output .= $Scholarships->$fieldsToExport[$i]."\t";
		    			}
	    				$output .= "\n";
	    			}
	    		...
	    		case 1:
	    			foreach ($checkbox as $check) {
			    		$this->model-> delete_scholarship($check);
			    	}
			    	return redirect()->back();
	    			break;
	    		default:
